                ...
                if "requestBody" in req_spec and "content" in req_spec["requestBody"]:
                    for content_spec in req_spec["requestBody"]["content"].values():
                        if "$ref" in content_spec["schema"]:
                            class_name = content_spec["schema"]["$ref"].removeprefix(
                                "#/components/schemas/"
                            )
                    ...
                    for response in req_spec["responses"].values():
                        if "content" in response:
                            for content_spec in response["content"].values():
                                if "$ref" in content_spec["schema"]:
                                    class_name = content_spec["schema"][
                                        "$ref"
    ...
    def serialize(self, template_file: str = "", **kwargs) -> str:
        ...
        for class_name in referenced_classes:
            json_schema = JsonSchemaGenerator(
                self.schema, include_null=False, top_class=class_name
            ).generate()
...
def cli(yamlfile, template, **args):
    ...
    openapi_spec = OpenApiGenerator(yamlfile, **args).serialize(
        template_file=template, **args
    )
    assert validate(yaml.safe_load(openapi_spec), cls=OpenAPIV30SpecValidator) is None
    print(openapi_spec, end="")
