...
> **Note:** See the [Custom Runtimes and Deployments](../extensibility/custom-runtimes-and-deployments.md) page for additional details.

A device MAY support custom runtimes and deployments by reporting `custom` for the `supportedRuntimes` and `supportedDeploymentTypes` properties. 

In order to enable application suppliers to target a device supporting a custom runtime and deployment type, the device capabilities MUST also report a set of `label` key/value pairs application suppliers can use to define eligibility rules in their Application Description.


## Specification Extensions

> **Note:** See the [specification extensions](../extensibility/specification-extensions.md) page for additional details.
